...
static int info(void) {
  ...
  if ((err = snd_mixer_selem_register(mhandle, smixer_level > 0 ? &smixer_options : NULL, NULL)) <
      0) {
...
struct volume_ops {
  ...
  int (*set)(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c, long value, int dir);
};
...
static int set_playback_dB(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c, long value,
                           int dir) {
  return snd_mixer_selem_set_playback_dB(elem, c, value, dir);
}

static int set_capture_dB(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c, long value,
                          int dir) {
  return snd_mixer_selem_set_capture_dB(elem, c, value, dir);
}

static int set_playback_raw_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                   long value, int dir) {
  return snd_mixer_selem_set_playback_volume(elem, c, value);
}

static int set_capture_raw_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                  long value, int dir) {
...
static int get_playback_mapped_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                      long *value) {
  //*value = (rint)(get_normalized_playback_volume(elem, c) * MAP_VOL_RES);
  return 0;
}

static int set_playback_mapped_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                      long value, int dir) {
...
static int get_capture_mapped_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                     long *value) {
  // *value = (rint)(get_normalized_capture_volume(elem, c) * MAP_VOL_RES);
  return 0;
}

static int set_capture_mapped_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t c,
                                     long value, int dir) {
...
static const struct volume_ops_set vol_ops[2] = {
    {
        ...
        .v =
            {
                {snd_mixer_selem_get_playback_volume_range, snd_mixer_selem_get_playback_volume,
                 set_playback_raw_volume},
                {snd_mixer_selem_get_playback_dB_range, snd_mixer_selem_get_playback_dB,
                 set_playback_dB},
                {get_mapped_volume_range, get_playback_mapped_volume, set_playback_mapped_volume},
            },
    },
    {
        ...
        .v =
            {
                {snd_mixer_selem_get_capture_volume_range, snd_mixer_selem_get_capture_volume,
                 set_capture_raw_volume},
                {snd_mixer_selem_get_capture_dB_range, snd_mixer_selem_get_capture_dB,
                 set_capture_dB},
...
static int set_volume_simple(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t chn, char **ptr,
                             int dir) {
...
static int show_control(const char *space, snd_hctl_elem_t *elem, int level) {
  ...
  case SND_CTL_ELEM_TYPE_INTEGER:
    printf(",min=%li,max=%li,step=%li\n", snd_ctl_elem_info_get_min(info),
           snd_ctl_elem_info_get_max(info), snd_ctl_elem_info_get_step(info));
    break;
  case SND_CTL_ELEM_TYPE_INTEGER64:
    printf(",min=%lli,max=%lli,step=%lli\n", snd_ctl_elem_info_get_min64(info),
           snd_ctl_elem_info_get_max64(info), snd_ctl_elem_info_get_step64(info));
    break;
  ...
  if (level & LEVEL_BASIC) {
    ...
    for (idx = 0; idx < count; idx++) {
      ...
      case SND_CTL_ELEM_TYPE_IEC958:
        ...
        printf("[AES0=0x%02x AES1=0x%02x AES2=0x%02x AES3=0x%02x]", iec958.status[0],
               iec958.status[1], iec958.status[2], iec958.status[3]);
        break;
      default:
        printf("?");
        break;
  ...
  __skip_read:
    ...
    if (type == SND_CTL_ELEM_TYPE_BYTES && !snd_ctl_elem_info_is_readable(info) &&
...
static void show_selem_volume(snd_mixer_elem_t *elem, snd_mixer_selem_channel_id_t chn, int dir,
                              long min, long max) {
  long raw, val;
...
static int show_selem(snd_mixer_t *handle, snd_mixer_selem_id_t *id, const char *space, int level) {
  ...
  if (level & LEVEL_BASIC) {
    ...
    if (snd_mixer_selem_has_capture_switch_exclusive(elem))
      printf("%sCapture exclusive group: %i\n", space, snd_mixer_selem_get_capture_group(elem));
    ...
    pmono =
        ...
        (snd_mixer_selem_is_playback_mono(elem) || (!snd_mixer_selem_has_playback_volume(elem) &&
                                                    !snd_mixer_selem_has_playback_switch(elem)));
    cmono = snd_mixer_selem_has_capture_channel(elem, SND_MIXER_SCHN_MONO) &&
            (snd_mixer_selem_is_capture_mono(elem) || (!snd_mixer_selem_has_capture_volume(elem) &&
...
static int selems_if_has_db_playback(int level, int include_mixers_with_capture) {
  int err;
  snd_mixer_t *handle;
  ...
  if ((err = snd_mixer_selem_register(handle, smixer_level > 0 ? &smixer_options : NULL, NULL)) <
      0) {
    error("Mixer register error: %s", snd_strerror(err));
    snd_mixer_close(handle);
    return err;
...
static int cset(int argc, char *argv[], int roflag, int keep_handle) {
  ...
  if (argc < 1) {
    fprintf(stderr, "Specify a full control identifier: "
                    "[[iface=<iface>,][name='name',][index=<index>,][device=<device>,][subdevice=<"
...
static const channel_mask_t chanmask[] = {
    ...
    {"front", ((1 << SND_MIXER_SCHN_FRONT_LEFT) | (1 << SND_MIXER_SCHN_FRONT_RIGHT))},
...
static int ctl_callback(snd_hctl_t *ctl, unsigned int mask, snd_hctl_elem_t *elem) {
...
int main(int argc, char *argv[]) {
  ...
  static const struct option long_option[] = {
      {"help", 0, NULL, 'h'},    {"card", 1, NULL, 'c'},       {"device", 1, NULL, 'D'},
      {"quiet", 0, NULL, 'q'},   {"inactive", 0, NULL, 'i'},   {"debug", 0, NULL, 'd'},
      {"nocheck", 0, NULL, 'n'}, {"version", 0, NULL, 'v'},    {"abstract", 1, NULL, 'a'},
      {"stdin", 0, NULL, 's'},   {"raw-volume", 0, NULL, 'R'}, {"mapped-volume", 0, NULL, 'M'},
      {NULL, 0, NULL, 0},
  };
  ...
  if (!strcmp(argv[optind], "help")) {
    ...
    return info() ? 1 : 0;
  } else if (!strcmp(argv[optind], "cards")) {
    return cards() ? 1 : 0;
  } else if (!strcmp(argv[optind], "pcms")) {
    return pcms(-1) ? 1 : 0;
